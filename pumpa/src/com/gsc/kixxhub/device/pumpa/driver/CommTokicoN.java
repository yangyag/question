...
import com.gsc.kixxhub.device.pumpa.translation.TransTokicoN;

public class CommTokicoN extends CCommDriver implements SerialPortEventListener, CommPortOwnershipListener, 
	Runnable {
	...
	protected int	baseReadStartInterval  = 15+4; // 스마트주유소 PJT 보정

	protected int	baseWriteStartInterval = 15+4; // 스마트주유소 PJT 보정
	...
	protected boolean recvedRealNozDownState=true;
    ...
    //public BytesQue TxQue = new BytesQue(10);
    ...
    public CommTokicoN (int nozNum, String romVerStr) {
		...
		try {
			...
			t10_ds.addString("basePrice", "1111", 4);
			...
			t11_ds.addString("basePrice1", "1111", 4);
			t11_ds.addByte  ("bPriceFlag2", (byte) '2');
			t11_ds.addString("basePrice2", "1111", 4);
			t11_ds.addByte  ("bPriceFlag3", (byte) '2');
			t11_ds.addString("basePrice3", "1111", 4);
			t11_ds.addByte  ("bPriceFlag4", (byte) '2');
			t11_ds.addString("basePrice4", "1111", 4);
			t11_ds.addByte  ("bPriceFlag5", (byte) '2');
			t11_ds.addString("basePrice5", "1111", 4);
			t11_ds.addByte  ("bPriceFlag6", (byte) '2');
			t11_ds.addString("basePrice6", "1111", 4);
	...
	public void processRecvSTX () throws SerialConnectException, Exception {
		...
		if (RxBuf[3]=='0' && RxBuf[4]=='0') { // 주유기 초기화
			...
			// Appended for 열린주유소(08/05/22)
			case 0 : // 노즐다운
				recvedRealNozDownState=true;
				break;
			case 1 : // 노즐업
				if (recvedRealNozDownState==true)
					nozState = 1;
				else
					nozState = 0;
				break;
			...
			return; // 오일파크 62전문 수신시 노즐다운 반복발생 문제로 추가(09/08/25)
		...
		case 3 :
			...
			if (nozState==0) { // 노즐다운
				...
				// 302 훼밀리 POS선결제 해제로 추가(09/10/17)
				if(romVer.equals("301") || romVer.equals("302")) { // 도끼꼬신형1
				...
				// 주유기 Status(주유자료) 요청->선사주유소 추가(2008/09/03) 하였으나
				// 주유기에서의 프리셋 설정이 풀리는 현상 발생으로 인하여
				// 동호주유소에서 삭제(2008/09/22)
				//TxQue.enQueue(t15_Buf);
			}
		...
		case 4 :
			if (nozState == 5) { // 주유완료후 토털게이지 수신
				...
				// Appended for 한국ENE(수원그린 #1,2,9,10)-2008.04.10
				if (m_nozLock==false && nozType==1) { // 일반주유기
					if (Change.toValue(m_basePrice) > 0)
						TxQue.enQueue(t10_Buf); // 주유허가(정액/정량 설정)
				}
				...
				S4_wm.setNozzleNo(Change.toString("%02d",nozNo));
				S4_wm.setBasePrice(m_basePrice); // 추가-선사(08/09/03)
				insertRecvQueue(S4_wm); // 주유완료 자료
	...
	public void requestData() throws Exception, SerialConnectException {
		...
		if (waitTotalGuageFor_S4==true) {
			if (waitGaugeForS4_Cnt > 15) {				
				...
				LogUtility.getPumpALogger().info("주유완료 토털게이지(65) 미수신, 주유완료(S4)전문 생성");
			}
			else
				waitGaugeForS4_Cnt++;
		...
		while (sndQue.getItemCount() > 0) {
			...
			// 302 훼밀리 POS선결제 해제로 추가(09/10/17)
			if (romVer.equals("302") && wm.getCommand().equals("PB"))
				wmByte[5] = '3'; // 주유기에서 변경불가
		...
		if (dispLevel>=3) 
			LogUtility.getPumpALogger().debug("\nStart request ========> TokicoN (Noz="+nozNo+")");
	...
	protected boolean verifyData (byte[] buf) throws Exception {
		...
		//System.out.printf("cmd=%c%c bufLen=%d", buf[3], buf[4], bufLen);
