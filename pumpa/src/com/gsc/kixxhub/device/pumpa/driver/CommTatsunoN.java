...
public class CommTatsunoN extends CCommDriver implements SerialPortEventListener, CommPortOwnershipListener, 
	...
	protected int		baseReadBuffInterval   = 30+6; // 스마트주유소 PJT 보정
	...
	public void processRecvSTX () throws SerialConnectException, Exception {
		...
		if (RxBuf[3]=='0' && RxBuf[4]=='0') { // 주유기 초기화
			...
			if (romVer.equals("152") || romVer.equals("153")) { // 152=초이 다쓰노신형1(똑딱+허가시점) (09/02/02)
																									// 153=금풍 다쓰노신형2(똑딱) (09/10/07)
				long currTime = cal.getTimeInMillis();
							
				if (nozState>=5 && state==1) {
					LogUtility.getPumpALogger().debug("nozzle="+nozNo+" -> Nozzle Up 무시함. nozState="+nozState+" state="+state);
					return; // 무시
				}
				...
				switch (state) {
					case 0 :  // 노즐다운
						if ((currTime - m_nNozUpTime) > 2000) { // 2 sec
							...
							//LogUtility.getPumpALogger().debug("nozzle="+nozNo+" -> Nozzle Down으로 인정함. currTime=" + currTime + " nozUpTime="+m_nNozUpTime);
							nozState=0;
						} else { // 무시함
							Sleep.sleep(500);
							TxQue.enQueueNewer(t15_Buf); // 상태정보요청
							nozState=1;
							LogUtility.getPumpALogger().debug("nozzle="+nozNo+" -> Nozzle Down으로 인정않음. currTime=" + currTime + " nozUpTime="+m_nNozUpTime);
							return;
						}
						break;
					case 1 :  // 노즐업
						if ((currTime - m_nNozDownTime) > 2000) { // 2 sec
							...
							//LogUtility.getPumpALogger().debug("nozzle="+nozNo+" -> Nozzle Up으로 인정함. currTime=" + currTime + " nozDownTime="+m_nNozDownTime);
							nozState=1;
						} else { // 무시함
							Sleep.sleep(500);
							TxQue.enQueueNewer(t15_Buf); // 상태정보요청
							LogUtility.getPumpALogger().debug("nozzle="+nozNo+" -> Nozzle Up으로 인정않음. currTime=" + currTime + " nozDownTime="+m_nNozDownTime);
							nozState=0;
							return;
						}
						break;
		...
		case 3 :
			...
			if (nozState==0) { // 노즐다운
				...
				// 용인문화(표준버전) 리모콘 조회불가로 수정->표준버전은 허가않음(09/09/16)
				// 초이(다쓰노신형1) POS선결제시 프리셋해제로 수정(09/09/23)
				if (romVer.equals("152") && m_nozLock==false && nozType==1) { // 일반주유기
				...
				if (romVer.equals("152")==false) { // 초이(다쓰노신형1) 노즐업시 프리셋해제 문제로 허가않음(09/09/07)
					if (m_nozLock==false && nozType==1) { // 일반주유기
						if (Change.toValue(m_basePrice) > 0) {
							TxQue.enQueue(t10_Buf); // 주유허가(통상주유)
							LogUtility.getPumpALogger().info("nozzle=" + nozNo + " -> 노즐업, 주유허가(e0) : progStep=" + progressStep + 
									" nozState=" + nozState + " m_basePrice=" + m_basePrice);
