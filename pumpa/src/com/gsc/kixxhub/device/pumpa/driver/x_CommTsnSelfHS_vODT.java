...
public class x_CommTsnSelfHS_vODT extends CCommDriver implements SerialPortEventListener, CommPortOwnershipListener, 
	Runnable {
    ...
    public x_CommTsnSelfHS_vODT (int nozNum, String romVerStr) {
...
/*			if (P1_sent==false && P1_wm != null) {
		...
		else if (wm.getCommand().equals("QM")) { // 승인응답

			QM_WoringMessage QM_wm = (QM_WoringMessage) wm;
			
			QM_wm.setNozzleNo(QM_wm.getNozzleNo());
			QM_wm.setTargetNozzleNo(QM_wm.getConnectNozzleNo());
			QM_wm.setPassThrough(false);

			insertRecvQueue(QM_wm); 
			
			LogUtility.getPumpALogger().info(new StringBuffer("\n다쓰노셀프(현성)ODT 수신전문 : " ).append( QM_wm.getCommand() )
		...
		else if (wm.getCommand().equals("QL")) { // 영수증 전문

			QL_WorkingMessage QL_wm = (QL_WorkingMessage) wm;
			
			QL_wm.setNozzleNo(QL_wm.getNozzleNo());
			QL_wm.setTargetNozzleNo(QL_wm.getConnectNozzleNo());
			QL_wm.setPassThrough(false);

			insertRecvQueue(QL_wm); 

			LogUtility.getPumpALogger().info(new StringBuffer("\n다쓰노셀프(현성)ODT 수신전문 : " ).append( QL_wm.getCommand() )
										 .append("\n ODT_No=" ).append( QL_wm.getNozzleNo() )
										 .append("\n nozzle=" ).append( QL_wm.getConnectNozzleNo() ).append("\n").toString());
			//HD_wm.print();
