...
public class TransPrimeSelf extends Translation {
	...
	public byte[] generateByteStream(WorkingMessage workingMessage) throws Exception {
		...
		if (workingMessageCommand.equals(IPumpConstant.COMMANDID_HD)) {
			...
			ss52DSt7.addString("trType", hdWorkingMessage.getTrType(), 1);		// 결제유형
			...
			// PrimeSelf 신형 52	 : 외상 외 응답 (trType 1,2,3,4,5,6,A,B,C,D,E,F,G,H,I) 
			...
			String vanMsg			= hcWorkingMessage.getVanMsg().trim();	// 밴사메시지
			String bnsMsg			= hcWorkingMessage.getBnsMsg().trim();	// 보너스메시지
			...
			ss52DS.addString("vanMsg", vanMsg, vanMsg.getBytes().length); // 밴사메시지
			ss52DS.addByte("FS2", IPumpConstant.DELIMITER_0X1C);
			ss52DS.addString("bnsMsg", bnsMsg, bnsMsg.getBytes().length); // 보너스메시지
	...
	public WorkingMessage generateWorkingMessage(byte[] message, 
		...
		if (somoCommandString.equals("51")) {
			...
			// PrimeSelf 신형 80 : 현금/신용/보너스/외상 승인/취소 요청 
			...
			// PrimeSelf 50 State 70 : 주유금지(비상정지)
			if (state.equals("70")) {
				paWorkingMessage.setNozzleState("0");					
				
			// PrimeSelf 50 State 80 : 비상해제 
			...
			if (divKey.equals("1"))	{
				...
				if (commandSet.equals("2")) {
					// PrimeSelf 2 (정액 설정) = PB_WorkingMessage 0
					commandSet = "0";
					
				} else if (commandSet.equals("0")) {
					// PrimeSelf 0 (Free 급유) = PB_WorkingMessage 2
					commandSet = "2";
...
/*
 * 
 * StatusCode 정의(POS팀 업무) 후 작업.
 * 
		} else if (somoCommandString.equals("91")) {
			// PrimeSelf 91 		 : 셀프 상태 통지   
			// WorkingMessage S8 : 
...
*/			
			...
			// PrimeSelf 90 		 : 정액 / 정량 마감 요청(정수 정지)
			...
			// PrimeSelf S3 		 : 정액 / 정량 마감 요청(정수 정지)
			// WorkingMessage S3 : 정액 / 정량 마감 요청(정수 정지)
			S3_WorkingMessage s3WorkingMessage = new S3_WorkingMessage();
			...
			// PrimeSelf S4 		 : 정액 / 정량 마감 요청(정수 정지)
			// WorkingMessage S4 : 정액 / 정량 마감 요청(정수 정지)
			S4_WorkingMessage s4WorkingMessage = new S4_WorkingMessage();
